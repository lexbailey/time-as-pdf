...
use chrono::prelude::*;
use pdf_writer::{types::LineCapStyle, Content, Finish, Name, Pdf, Rect, Ref, Str};
...
fn clockface(content: &mut Content, now: DateTime<FixedOffset>) {
    ...
    radial_line(content, 2. * PI / 60. * now.hour() as f32, 0., 48.);
...
fn time_as_pdf(url: &str, offset: i32) -> Vec<u8> {
    ...
    txt!(
        ...
        100.0,
        14.0,
        format!("Generated by the URL: {}", url).as_bytes()
    );
...
impl<'r> FromRequest<'r> for ReqInfo {
    ...
    async fn from_request<'l0>(r: &'r Request<'l0>) -> request::Outcome<Self, Self::Error> {
        rocket::outcome::Outcome::Success(ReqInfo {
            uri: format!("{}", r.uri()),
        })
    }
}
...
#[launch]
fn rocket() -> _ {
    rocket::build().mount("/", routes![index, humans, utcplus, utcminus])
}
